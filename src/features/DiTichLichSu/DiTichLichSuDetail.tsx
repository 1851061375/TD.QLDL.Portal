...
const fetchData = async (id: any) => {
  const res = await requestGET(`ditichlichsus/${id}`);
  return res;
};
...
const DiTichLichSuDetail: FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      let loaiid = res?.data?.LoaiDiTichLichSuDetailID;
      if (loaiid) {
      ...
      >
        {state.item && (
          <div style={{ maxWidth: 1200, margin: "auto" }}>
            ...
            <Title level={4}>Di tích lịch sử khác</Title>
            <List
              grid={{
                gutter: 16,
                xs: 1,
...
export default DiTichLichSuDetail;
