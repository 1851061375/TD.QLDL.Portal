...
const fetchData = async (id: any) => {
  const res = await requestGET(`khudiemdulichs/${id}`);
  return res;
};

const fetchSimilarData = async (typeId: any, woids: any) => {
  const res = await requestGET(`khudiemdulichs`, {
    loaiid: typeId,
    woids: woids,
    page: 1,
    offset: 4,
...
const KhuDiemDuLichDetail: FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      let loaiid = res?.data?.LoaiKhuDiemDuLichDetailID;
      if (loaiid) {
      ...
      >
        {state.item && (
          <div style={{ maxWidth: 1200, margin: "auto" }}>
            ...
            <Title level={4}>Khu điểm du lịch khác</Title>
            <List
              grid={{
                gutter: 16,
                xs: 1,
...
export default KhuDiemDuLichDetail;
