...
const fetchData = async (id: any) => {
  const res = await requestGET(`danhlamthangcanhs/${id}`);
  return res;
};

const fetchSimilarData = async (typeId: any, woids: any) => {
  const res = await requestGET(`danhlamthangcanhs`, {
    loaiid: typeId,
    woids: woids,
    page: 1,
    offset: 4,
...
const DanhLamThangCanhDetail: FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      let loaiid = res.data?.LoaiDanhLamThangCanhDetailID;
      if (loaiid) {
      ...
      >
        {state.item && (
          <div style={{ maxWidth: 1200, margin: "auto" }}>
            ...
            <Title level={4}>Danh lam thắng cảnh khác</Title>
            <List
              grid={{
                gutter: 16,
                xs: 1,
...
export default DanhLamThangCanhDetail;
