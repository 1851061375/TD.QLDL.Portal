...
import {
  Select, Input, Button, List, Pagination, Spin,
  TimePicker
} from "antd";
...
export const ItemList = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const res = await fetchData(
        ...
        state.offset,
        state.search,
        state.diaBanSelected,
        state.ngayMoCuaSelected
      );
      setState((prevState) => ({
        ...prevState,
        data: res.data ?? [],
  ...
  const diaBanOptions = useMemo(
    () =>
      state.diaBan?.map((filterItem: any) => (
  ...
  const ngayMoCuaOptions = useMemo(
    () =>
      state.ngayMoCua?.map((filterItem: any) => (
