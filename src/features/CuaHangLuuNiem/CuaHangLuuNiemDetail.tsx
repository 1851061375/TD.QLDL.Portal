...
const fetchData = async (id: any) => {
  const res = await requestGET(`cuahangluuniems/${id}`);
  return res;
};

const fetchSimilarData = async (typeId: any, woids: any) => {
  const res = await requestGET(`cuahangluuniems`, {
    loaiid: typeId,
    woids: woids,
    page: 1,
    offset: 4,
      ...
      >
        {state.item && (
          <div style={{ maxWidth: 1200, margin: "auto" }}>
            ...
            <Title level={4}>Cửa hàng lưu niệm khác</Title>
            <List
              grid={{
                gutter: 16,
                xs: 1,
...
export default CuaHangLuuNiemDetail;
